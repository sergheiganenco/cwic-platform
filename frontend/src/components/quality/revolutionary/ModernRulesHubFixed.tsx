...
const ModernRulesHub: React.FC<ModernRulesHubProps> = ({
      ...
      action: async () => {
        ...
        if (!effectiveDataSourceId) {
          if (dataSources.length > 0) {
            ...
            console.log('📝 "All Servers" selected for autopilot, using first available data source:', dataSources[0].name);
          } else {
        ...
        toast.loading('Analyzing your data patterns...', { id: 'autopilot' });

        try {
          const response = await axios.post('/api/quality/autopilot/enable', {
            dataSourceId: effectiveDataSourceId
          });
          toast.success('Autopilot is generating rules for you!', { id: 'autopilot' });
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const apiRules = rulesArray.map((rule: any) => ({
          ...
          category: rule.dimension || rule.parameters?.dimension || 'validity',
          type: rule.type || rule.rule_type || 'custom',
          status: rule.enabled ? 'active' : 'paused',
          severity: rule.severity || 'medium',
          lastRun: rule.last_run_at,
          successRate: rule.success_rate || 0,
          affectedAssets: rule.affected_assets || 0,
          ...
          performance: {
            avgExecutionTime: rule.avg_execution_time || 0,
            issuesFound: rule.issues_found || 0,
            issuesResolved: rule.issues_resolved || 0,
            trend: rule.trend || 'stable'
          }
        }));
        setRules(apiRules);
      } else {
  ...
  const startAIConversation = useCallback(() => {
    ...
    // Build personalized greeting based on memory
    const hasHistory = aiMemory.conversationHistory.length > 0;
    const lastRule = aiMemory.userPreferences.lastRuleCreated;
    const personalizedIntro = hasHistory && lastRule
      ? `\n\n✨ **I remember you!** Last time you created: *"${lastRule.name}"*\nWant to create something similar?`
      : '';

    // AI greets user with enhanced intelligence and context
    setTimeout(() => {
      const contextInfo = dataSourceId ? '\n\n🔗 **Current Context:** Working with your connected data source' : '';
      const tablesInfo = dataTables.length > 0 ? `\n📋 **Available tables:** ${dataTables.slice(0, 5).join(', ')}${dataTables.length > 5 ? '...' : ''}` : '';

      setChatMessages([{
        role: 'ai',
        content: `Hello! I'm your AI Data Quality Assistant 🧠\n\nI'm here to help you create intelligent quality rules. I understand natural language and ask clarifying questions when needed.${contextInfo}${tablesInfo}${personalizedIntro}\n\n**Tell me what you want to do:**\n• *"Scan all sources for SSN"*\n• *"Check for missing customer emails"*\n• *"Find duplicate orders by order_id"*\n• *"Validate that prices are positive numbers"*\n\n💡 **Or ask me questions:**\n• *"What data sources are available?"*\n• *"What tables do we have?"*\n• *"Show me PII detection options"*\n\nI'm listening! What would you like to do?`,
        timestamp: new Date()
      }]);
    }, 500);
  ...
  const handleAIChat = async () => {
    ...
    const detectIntent = (text: string) => {
      ...
      // PII detection with "all" support
      if (/(scan|check|find|detect).*(all|every|entire).*(source|table|database)/i.test(text) && /ssn|pii|sensitive/i.test(text)) {
        return 'scan_all_pii';
      }
      if (/ssn|social\s*security|taxpayer|tin\b/i.test(text)) return 'ssn';
      if (/credit\s*card|cc\s*num|card\s*number/i.test(text)) return 'credit_card';
      if (/pii|sensitive|personal.*info|5/i.test(text)) return 'pii';

      if (/complete|missing|null|empty|blank|1/i.test(text)) return 'completeness';
      if (/valid|format|email|phone|pattern|2/i.test(text)) return 'validity';
      if (/unique|duplicate|dupl|3/i.test(text)) return 'uniqueness';
      if (/accura|range|between|limit|4/i.test(text)) return 'accuracy';
    ...
    switch(currentStep) {
      case 0: // Initial - Handle questions AND intents
        ...
        if (/what.*(table|schema)/i.test(input)) {
          ...
          // User is asking about data sources/tables
          const sourceCount = availableDataSources.length;
          const sourceList = sourceCount > 0
            ? availableDataSources.map(s => `• **${s.name}** (ID: ${s.id})`).join('\n')
            : '• Connected to current data source';

          aiResponse = `📊 **Available Data Sources:**\n${sourceList}\n\n${dataSourceId ? `✓ Currently working with: **${dataSourceId}**\n\n` : ''}Would you like to:\n• Scan **all sources** for PII/quality issues?\n• Target a **specific source**?\n• Check **specific tables** (I can discover them)?\n\nJust tell me what you need!`;
          // Stay at step 0
          ...
          const sourceInfo = availableDataSources.length > 0
            ? `all ${availableDataSources.length} connected data sources`
      ...
      case 1: // Table/scope/refinement
        if (isContextQuery || isQuestion) {
          ...
          if (/what.*(column|field|table)/i.test(input)) {
            ...
            const sources = availableDataSources.length > 0
              ? availableDataSources.map(s => `• ${s.name}`).join('\n')
          ...
          // Process scope input
          const isAllSources = /(all|every|entire|whole).*(source|table|database)/i.test(input);
          ...
          <div className="p-4 border-t border-gray-200 bg-white">
            <div className="flex items-end gap-2">
              ...
              <button
                ...
                className="px-6 py-3 bg-gradient-to-r from-violet-500 to-purple-600 text-white rounded-xl hover:shadow-lg transform hover:scale-105 transition-all flex items-center gap-2 font-medium disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
              >
                <Send className="w-4 h-4" />
                Send
              </button>
            </div>
            <p className="text-xs text-gray-400 mt-2">Press Enter to send • Step {currentStep + 1} of 5</p>
          </div>
        </motion.div>
      </motion.div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <AnimatePresence>
        {AIAssistantModal}
        {showTemplates && <TemplatesGallery />}
        {selectedRule && <RuleDetailsModal rule={selectedRule} />}
        {showScanResults && <ScanResultsModal />}
